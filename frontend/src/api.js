...
export const api = {
  ...
  getSongs: (signal, status, page, page_size, search) => instance.get('/songs/', { params: { status, page, page_size, search: search || undefined }, signal }).then(r => r.data),
  ...
  manualDownload: (url, allow_playlist = false, override_duplicate = false, no_purge = false) => instance.post('/jobs/manual/', { url, allow_playlist, override_duplicate, no_purge }).then(r => r.data),
