  ...
  return (
    <div style={{ background: 'var(--surface2)', padding: 16, borderRadius: 8 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
        <div>
          <h4 style={{ margin: 0, fontSize: 14 }}>Purge Analysis</h4>
          <span style={{ fontSize: 11, color: 'var(--text-dim)' }}>Explicitly showing how songs are matched for protection.</span>
        </div>
        <button onClick={load} style={refreshBtnStyle}>
          <RefreshCw size={14} style={{ marginRight: 6 }} /> Re-Analyze
        </button>
      </div>

      {loading ? <div>Analyzing library and playlists...</div> : (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
          ...
          {data.debug_info && (
            <div style={{ background: 'rgba(155,81,224,0.05)', border: '1px solid var(--accent)', padding: 10, borderRadius: 4, fontSize: 11 }}>
              <div style={{ fontWeight: 800, marginBottom: 4, display: 'flex', alignItems: 'center', gap: 6 }}>
                <Info size={12} /> SYSTEM DEBUG INFO
              </div>
              <div>Monitored Playlists: {data.debug_info.monitored_playlists}</div>
              <div>Tracks Found in Playlists: {data.debug_info.total_playlist_tracks}</div>
            </div>
          )}
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
            ...
            <div>
              <div style={{ ...sectionLabel, color: 'var(--red)' }}>Deletion Candidates ({data.candidates.length})</div>
              <div style={listContainer}>
                {data.candidates.length === 0 && <div style={emptyStyle}>No candidates for deletion.</div>}
            ...
            {/* Protected Songs */}
            <div>
              <div style={{ ...sectionLabel, color: 'var(--green)' }}>Protected / Skipped ({data.protected.length})</div>
              <div style={listContainer}>
                {data.protected.length === 0 && <div style={emptyStyle}>No protected songs detected.</div>}
                {data.protected.map((u, i) => (
                  <div key={i} style={itemStyle}>
                    <ShieldCheck size={12} color="var(--green)" style={{ flexShrink: 0 }} />
                    <div style={{ flex: 1, overflow: 'hidden' }}>
                      <div style={filenameStyle}>{u.filename}</div>
                      <div style={{ fontSize: 10, color: 'var(--accent)' }}>Matched via: {u.match_reason}</div>
                      <div style={{ fontSize: 10, color: '#aaa' }}>Playlists: {u.playlists.join(', ')}</div>
                    </div>
                  </div>
                ))}
              </div>
