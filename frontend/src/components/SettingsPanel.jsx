...
export const SettingsPanel = ({ onUpdate, notify, onLogout }) => {
  ...
  const handleBackgroundUpload = async (e) => {
    ...
    try {
      ...
      notify("Background updated! Please refresh the page to see changes on the login screen.");
    } catch (err) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div style={{ background: 'var(--surface2)', padding: 24, borderRadius: 8 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
          <div style={inputGroup}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              <label style={{ ...saveBtnStyle, background: 'var(--surface)', border: '1px solid var(--border)', cursor: 'pointer', flex: 1 }}>
            ...
            <div style={{fontSize: 10, color: 'var(--text-dim)'}}>Will be shown on the login screen.</div>
          </div>
          ...
          <div style={inputGroup}>
            ...
            <button 
              onClick={onLogout}
              style={{ ...saveBtnStyle, background: 'transparent', border: '1px solid #ff4d4d', color: '#ff4d4d', flex: 1 }}
            >
        ...
        <div style={{ marginTop: 20, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <input 
              type="checkbox" 
              id="showDashboardBg"
              checked={config.UI_DASHBOARD_BG === 'true'} 
              onChange={e => setConfig({...config, UI_DASHBOARD_BG: e.target.checked ? 'true' : 'false'})}
              style={{ width: 18, height: 18, cursor: 'pointer' }}
            />
            <label htmlFor="showDashboardBg" style={{ ...labelStyle, fontSize: 13, cursor: 'pointer' }}>
              {t('settings.show_dashboard_bg')}
            </label>
          </div>
          ...
          <div style={inputGroup}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
                <input 
                    type="color"
                    value={config.UI_THEME_COLOR || '#9b51e0'}
                    onChange={e => setConfig({...config, UI_THEME_COLOR: e.target.value})}
