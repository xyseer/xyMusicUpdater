...
def _ytdlp_download(url, dest, label, max_items=10, job=None, allow_playlist=True):
    ...
    cfg = _cfg()
    cmd_meta = ["yt-dlp", "--flat-playlist", "--dump-json", "--playlist-end", str(max_items)]
    ...
    for title, vid in targets:
        ...
        cmd = ["yt-dlp", "-x", "--audio-format", "mp3", "--audio-quality", "0", "--no-mtime", "--no-overwrites", "--add-metadata", "--embed-thumbnail", "--output", output_tpl]
